...
async function searchJobs(query, filters, signal) {
  ...
  const results = [];
  let failure;
  const pages = Array.from({ length: JOB_PAGES }, (_, i) => fetchJobPage(params, i + 1, pageSignal).then(
    jobs => { results[i] = jobs; },
    error => { failure = failure || error; }
  ));
  await new Promise(resolve => {
    pages.forEach((page, i) => page.then(() => { if (results[i]?.length) resolve(); }));
    Promise.all(pages).then(resolve);
  });
  await Promise.race([Promise.all(pages), new Promise(resolve => setTimeout(resolve, PAGE_GRACE))]);
  stragglers.abort();
...
// scoring starts as soon as the listings arrive, in parallel batches.
router.post('/find-matches', apiAuth, async (req, res) => {
  ...
  const { query = '', cvText = '', filters = {} } = req.body;
  ...
  try {
    ...
    await eachLimited(batches, SCORING_CONCURRENCY, async batch => {
      let matches = [];
      try {
        matches = await scoreBatch(cvText, batch, signal);
